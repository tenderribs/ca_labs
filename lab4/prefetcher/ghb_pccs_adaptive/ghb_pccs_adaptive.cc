...
void ghb_pccs_adaptive::prefetcher_cycle_operate()
{
  epoch_cycle_count++;
...
bool ghb_pccs_adaptive::pointer_valid(const uint16_t& pointer, const uint16_t& tag) const
{
  // 1. Null check
...
uint16_t ghb_pccs_adaptive::sanitize_pointer(const uint16_t& pointer, const uint16_t& tag) const { return pointer_valid(pointer, tag) ? pointer : INVALID_PTR; }
